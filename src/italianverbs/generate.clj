...
(defn head-by-comps [parent head comps depth]
  ...
  (if (not (empty? comps))
    (let [comp (first comps)
      ...
      (log/debug (str "COMP-IS-FINISHED? : " comp-is-finished?))
      (if comp-is-finished?
      ...
      (cond (and (not (nil? head-expand))
            ...
            (let [shortcut
                  (if head-is-finished?
                    ...
                    :top)
                  log-shortcut
                  (do (log/debug (str "shortcut: " shortcut))
                      (log/debug (str "parent: " (unify/get-in parent '(:comment-plaintext))))
                      (log/debug (str "shortcut head: " (if head-is-finished? (fo head)))))
               ...
               (let [result (unify-lr-hc parent head comp-specification)]
                 ...
                 (if (unify/fail? result)
                   (do
                     ...
                     (log/info (str "<debug>"))
                     (unify-lr-hc-debug parent head comp-specification)
                     (log/info (str "</debug>"))
                     (head-by-comps parent head (rest comps) depth))
                   (do
