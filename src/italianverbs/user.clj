...
(defn login-form [request]
  (let [authentication (haz-auth? request)]
    (if authentication
      (authentication/logged-in-content request)
      ...
      (html
       [:div {:class "login major"}
        ...
        (if (:allow-internal-authentication env)
          (html
           [:div {:style "margin-top:0.5em;border-top:1px solid #ccc"}
...
(defn do-if-teacher-or-admin [what-to-do & [else]]
  (do-if (or (has-admin-role) (has-teacher-role))
         (do (log/debug (str "Authorized attempt to access a teacher-and-admin-only function."))
             what-to-do)
         (if else else
             (do (log/warn (str "Unauthorized attempt to access a teacher-and-admin-only function."))
                 {:status 302
