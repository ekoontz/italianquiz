(ns italianverbs.morphology.français
...
(defn get-string-1 [word & [ {usted :usted
  ...
  (let [person (get-in word '(:agr :person))
    ...
    (log/debug (str "word's a français is a string? " (get-in word '(:a :français)) " => " (string? (get-in word '(:a :français)))))
