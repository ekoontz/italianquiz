...
(deftest get-trees-test-with-ref
  (let [fs {:a (ref (set (list 1 2)))}
    ...
    (is (set? trees))
    (is (= (.size trees) 2))
    (is (ref? (:a (first trees))))
    (is (ref? (:a (second trees))))

    ;; ..but they are not the *same* ref:
    (is (not (= (:a (first trees))
                (:a (second trees)))))

    ;; check values of the two refs:
    (is (or (= 1 (get-in (first trees) '(:a)))
            (= 2 (get-in (first trees) '(:a)))))
